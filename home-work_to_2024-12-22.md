...
### Для нубиков, которые не сделали на уроке
...
Также нужно обозначить имя для грид-области, которую будет занимать заголовок: `grid-area: title;`.
...
Изображение должно занимать все доступное пространство по ширине (свойство `width`). Если используете разные изображения для разных карточек, то убедитесь, что они одинакового размера, иначе жестко задавайте всем изображениям высоту (это некрасиво, но чо поделать).
