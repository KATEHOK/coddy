...
Теперь мы с вами будем в каждый нащ css-файл добавлять в начало следующий фрагмент кода:
...
Напомню, что css-селектор `*` применяет стили ко всем объектам станицы. Этот селектрор имеет наименьший приоритет, что позволяет при необходимости зпросто переопределять указанные с ним стили.

Данный блок необходим для удобства нашей стилизации. Нулевые `margin` и `padding` позволят не забыть убрать где-то лишний отступ, так как по умолчанию все они будут отключены. `box-sizing: border-box;` очень часто будет нам необходим, поэтому имеет смысл применить его по умолчанию ко всем объектам.

#### Основной цвет фона

В предыдущей домашке мы никак не меняли цвет фона для объекта `body`, давайте же это исправим. Измените цвет фона на какой-нибудь легкий светнлый оттенок, подбирайте его так, чтобы черный текст на его фоне легко читался и не резал глаза.

#### Шрифт по умолчанию с Google Fonts

Зайдите на Google Fonts и отыщите красивый шрифт. Особых требований к нему я не предъявляю за исключением одного: шрифт должен поддерживать как латиницу (английские буквы), так и кириллицу (русские буквы).

Подключите выбранный шрифт к файлу `example.html`, а в файле стилей `example.css` примените его, как шрифт по умолчанию для всех лбъектов страницы (селектор `*`).

#### Главный заголовок

Увеличте размер шрифта главного заголовка (не меньше 56 пикселей).

#### Карточка товара

В прошой домашке мы добавили карточки доваров, давайте их немного стилизуем.

##### Фон карточки

Измените цвет фона карточки, причем если общий цвет фона имел холодный оттенок, то тогда цвет фона карточки должен иметь теплый оттенок - и наоборот. Помните, что установленный цвет фона карточки не должен резать глаза, а напечатанный текст должен быть легко читаем.

##### Рамка карточки

Измените рамку карточки. В прошлый раз мы делали ее толщиной 1 пиксель сплошной черного цвета. Пусть теперь она будет потолще, не обязательно сплошной и желательно другого цвета.

##### Внутренние отступы карточки

Увеличте внутренние отступы с 10 пикселей до 20.

##### Скругления углов карточки

Добавьте скругрения углов карточки, желательно сделать их симметричными.

##### Заголовок карточки

Измените размер шрифта заголовка карточки. Сделайте его размером не меньше, чем 28 пикселей.

##### Текст карточки

В текстовые блоки карточек добавьте побольше текста (Не меньше 100 слов). Измените размер шрифта текстовых блоков карточек (не меньше, чем 18 пикселей, но меньше, чем размер шрифта заголовка карточки).

##### Картинка карточки

Уменьшите ширину картинки. Теперь она должна занимать не более 20 процентов от ширины самой карточки. Добавьте для картинки обтекание текстом, при этом картинка должна быть "приклеена" к левому краю карточки. Чтобы текст не прилипал к картинке, добавьте картинке внешний правый отступ размером в 10 пикселей.

#### Результат

Если вы все сделали согласно требованиям, у вас могло получиться что-то похожее:

![example](./pics/2024-11-10_example.png)

По желанию можете добавить и прочие элементы оформления текста, указанные в теоретическом блоке.
